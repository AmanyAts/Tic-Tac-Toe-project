...
if(!isCom){
  ...
  }while(document.querySelectorAll(".box")[random].innerHTML!="")
  console.log('raa '+random);
  document.querySelectorAll(".box")[random].innerHTML='<strong class="XandO">' + o + "</strong>";
...
function FindWinner(event) {
  ...
  for (let i = 0; i < playing.length; i++) {
    ...
    if (i == 0 || i == 1 || i == 2) {
      //column cases
      if (playing[i] == playing[i + 3] &&playing[i] == playing[i + 6] &&playing[i] != null) {
        swal("player " + player + " is winner");
        $("div").off("click");
        return true;
      }
  ...
  if (playing[0] == playing[4] &&playing[0] == playing[8] &&playing[0] != null) {
    ...
    $("div").off("click");
    return true;
  } else if (playing[2] == playing[4] &&playing[2] == playing[6] &&playing[2] != null) {
    //diagnols
    swal("player " + player + " is winner");
